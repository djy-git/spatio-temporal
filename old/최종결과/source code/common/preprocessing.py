...
def feature_engineering(data, encode_TurbID=False, compute_Pmax_method='simple', compute_Pmax_clipping=True, power_constant=0.5):
    ...
    temp.Tmstamp2 = temp.Tmstamp.apply(lambda x: int(x.split(':')[0]) + int(x.split(':')[1]) / 60)
    temp['TimeX'] = np.cos(2 * np.pi * (temp.Tmstamp2 / 24))
    temp['TimeY'] = np.sin(2 * np.pi * (temp.Tmstamp2 / 24))
    ...
    ## weekday
    temp['Weekday'] = temp['Day'].apply(lambda x: x % 7)
    
    ## month
    temp['Month'] = temp['Day'].apply(lambda x: x // 30)
    ...
    temp['RPM5'] = temp['RPM'].shift(144*5)
    
    temp['WspdMA3'] = temp['Wspd'].rolling(144*3).mean()
    temp['PatvMA3'] = temp['Patv'].rolling(144*3).mean()
    temp['RPMMA3'] = temp['RPM'].rolling(144*3).mean()
    
    for i in range(1,7):
        temp[f'Etmp{i}'] = temp['Etmp'].shift(144*i)
        temp[f'EtmpMA{i}'] = temp['Etmp'].rolling(144*i).mean()
